...
app.use('/', require('../public/API/api'));
connectDB();
...
app.get('/quiz-creator', function(req, res){
    res.render('quiz-creator')
})

app.get('/choosenho', function(req, res){
    res.render('choosenho')
})
...
app.get('/createhandout', function(req, res){
    res.render('createhandout')
})

app.get('/viewtext', function(req, res){
    res.render('viewtext')
})

app.get('/practicemode', function(req, res){
    res.render('practice')
})

app.get('/practicegame', function(req, res){
    res.render('practicegame')
})

app.get('/allclasses', function(req, res){
    res.render('allclasses')
})
...
io.on('connection', (socket) => {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection('kahootGames').find(query).toArray(function(err, result){
                ...
                if(result[0] !== undefined){
                    ...
                    games.addGame(gamePin, socket.id, false, {playersAnswered: 0, questionLive: false, gameid: data, question: 1}); //Creates a game with pin and host id
    ...
    socket.on('playerAnswer', function(num){
        ...
        if(game.gameData.questionLive == true){//if the question is still live
            ...
            MongoClient.connect(url, function(err, db){
                ...
                dbo.collection("kahootGames").find(query).toArray(function(err, res) {
                    if (err) throw err;
                    var correctAnswer = res[0].questions[gameQuestion - 1].answers[0]
    ...
    socket.on('requestDbNames', function(userid){
        ...
        MongoClient.connect(url, function(err, db){
            ...
            dbo.collection("kahootGames").find().toArray(function(err, res) {
                if (err) throw err;
                res = res.filter(elements => elements.userid===userid)
    ...
    socket.on('getHandout', function(id){
        ...
        MongoClient.connect(url, function(err, db){
            ...
            collection.find().toArray(function(err, res) {
                res = res.filter(elements => elements._id==id)
              
                if (err) throw err;
                socket.emit('getHandout', res);
                db.close();
            });
        });
    ...
    socket.on('requestHandouts', function(userid){
        MongoClient.connect(url, function(err, db){
            if (err) throw err;
            ...
            dbo.collection("files").find().toArray(function(err, res) {
                res = res.filter(elements => elements.userid==userid)
                if (err) throw err;
                socket.emit('requestHandouts', res);
                db.close();
            });
        });
    });
    ...
    socket.on('getclassesofuser', function(data){
        MongoClient.connect(url, function(err, db){
            if (err) throw err;
            var dbo = db.db('DatabaseDA');
            dbo.collection("classes").find().toArray(function(err, res) {
                res = res.filter(elements => elements.userid==data)
                if (err) throw err;
                socket.emit('getbackclasses', res);
                db.close();
            });
        });
    });
