...
socket.on('connect', function(){
    ...
    socket.emit('searchforDBvocaswithID', id);
});

socket.on('gobacktovocas', function(){
...
socket.on('gameQuestiondata', function(data){
    ...
    data[0].questions.forEach(element => {
        if(questionNum===0){
            document.getElementById("q1").value=element.question;
            document.getElementById("1a1").value=element.answers[0];
            questionNum++;
        }
        else{
            addQuestionofgameinDB(element.question, element.answers[0])
        }
    });
});
...
function updateDatabase(){
    ...
    var quiz = {"id":0 ,"name": name, "questions": questions, "idofold":  window.location.href.split('=')[1]};
    socket.emit('updateexisingquiz', quiz);
