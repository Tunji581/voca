...
socket.on("getbackclasses", async function(data){
    var classesinfile = document.getElementById("classes")
 ...
 for (let index = 0; index < data.length; index++) {
    ...
    var indetail = document.createElement("h2");
    indetail.setAttribute("class", "main-title")
    indetail.innerHTML(element.classname);
    summ.appendChild(indetail);
    
    classesinfile.appendChild(details)
...
socket.on('gameNamesData', async function(data){
 ...
 socket.emit('requestHandouts', localStorage.getItem("userid"));
    for(var i = 0; i < 8; i++){
